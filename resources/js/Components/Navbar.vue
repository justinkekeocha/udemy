...
<template>
  <nav class="flex flex-row flex-wrap gap-x-5 p-1 bg-white border-gray-200 dark:bg-gray-900 shadow-md items-center">
    ...
    <div class="flex items-center -ml-5">
      ...
      <Popover target="categories" class="z-50">
        <ul
          ...
          <span v-for="row in categoryStore.categories">
            <li class="w-full px-4 py-2 hover:text-purple-600" :data-dropdown-toggle="'subCategory' + row.id + 'Dropdown'"
              data-dropdown-placement="right-start" data-dropdown-trigger="hover" data-dropdown-offset-distance="15">
      ...
      <div v-for="(subCategory, index) in subCategoryStore.groupedSubCategories">
        <div :id="'subCategory' + index + 'Dropdown'"
          class="z-50 hidden bg-white divide-y divide-gray-100 shadow w-44 dark:bg-gray-700" ref="subCategory">
